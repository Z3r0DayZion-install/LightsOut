...
function Show-SnoozeDialog {
    ...
    # Snooze button
    $snoozeBtn = New-RoundedButton -Text "ðŸ˜´ Snooze $SnoozeMinutes min" -Size (New-Object System.Drawing.Size(180, 45)) `
    ...
    # Proceed button
    $proceedBtn = New-RoundedButton -Text "âœ“ Proceed Now" -Size (New-Object System.Drawing.Size(180, 45)) `
...
function New-SleepTimerForm {
    ...
    $themeBtn.Text = if ($script:IsDarkTheme) { "â˜€" } else { "â˜¾" }
    ...
    $profileCombo = New-ModernComboBox -Size (New-Object System.Drawing.Size(180, 35)) -Location (New-Object System.Drawing.Point(30, 220)) `
        -Items ($profiles.PSObject.Properties.Name | ForEach-Object { "âš¡ $_" }) `
        -SelectedItem "âš¡ $script:CurrentProfile"
    ...
    # Main Action Buttons
    $startButton = New-RoundedButton -Text "â–¶  START TIMER" -Size (New-Object System.Drawing.Size(220, 50)) `
        -Location (New-Object System.Drawing.Point(30, 410)) -BackColor $script:Colors.Success
    $script:StartButton = $startButton
    $form.Controls.Add($startButton)
    
    $cancelButton = New-RoundedButton -Text "â¹  CANCEL" -Size (New-Object System.Drawing.Size(220, 50)) `
...
function Show-SettingsDialog {
    ...
    $autoStartCheck.Text = "âš¡ Auto-start with Windows (tray mode)"
    ...
    # Save Button
    $saveBtn = New-RoundedButton -Text "ðŸ’¾ Save" -Size (New-Object System.Drawing.Size(150, 40)) `
...
function Show-HistoryDialog {
    ...
    $clearBtn = New-RoundedButton -Text "ðŸ—‘ Clear History" -Size (New-Object System.Drawing.Size(140, 35)) `
...
if ($ListProfiles) {
    ...
    foreach ($profileName in $profiles.PSObject.Properties.Name) {
        ...
        Write-Host "  â€¢ $profileName : $($data.Minutes) minutes â†’ $($data.Action)" -ForegroundColor White
    }
    Write-Host ""
    exit 0
}
...
if ($ExportSettings) {
    try {
        ...
        Write-Host "âœ“ Settings exported to: $ExportSettings" -ForegroundColor Green
        exit 0
    }
    catch {
...
if ($ImportSettings) {
    try {
        ...
        Write-Host "âœ“ Settings imported from: $ImportSettings" -ForegroundColor Green
        exit 0
    }
    catch {
...
if ($ScheduleTime) {
    try {
        ...
        if (-not $Silent) {
            Write-Host "â° Scheduled for $ScheduleTime ($waitMinutes minutes from now)" -ForegroundColor Cyan
        }
    }
    catch {
...
if ($NoGUI) {
    ...
    try {
        ...
        if (-not $Silent) {
            Write-Host "`nâœ“ Timer cancelled successfully." -ForegroundColor Green
        }
    }
}
else {
