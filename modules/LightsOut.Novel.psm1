...
function Get-MorningProofReport {
    ...
    foreach ($line in Get-Content $AuditLogPath -ErrorAction SilentlyContinue) {
        ...
        try { $dt = [DateTime]::Parse($Matches[1]) } catch { continue }
    ...
    if ($LastSeen) {
        try {
            $seenAt = [DateTime]::Parse($LastSeen)
            if ($term.Ts -le $seenAt) { $showProof = $false }
        } catch { }
    }
    ...
    switch ($state) {
        'completed' {
            ...
            $detail = $heroTag
            $encourage = 'Great job. You kept your promise. Rest well. You''ve got tomorrow.'
